...
- Роль мониторинга по умолчанию шлёт уведомления в Telegram. Почтовая доставка (установка `mailutils` + `exim4`) включается переменной `monitor_mail_enabled=true`; после этого дополнительные получатели добавляются через `monitor_mail_to`.
