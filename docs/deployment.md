...
6. Поднимает `postgres` без выделенного тома: данные базы лежат внутри контейнера, при пересоздании контейнера база и схема поднимаются с нуля из `src/create_tables.sql`.
...
- PostgreSQL работает без собственного Docker-тома: данные живут в слое контейнера, поэтому при проливе нового контейнера база инициализируется заново (создаётся `POSTGRES_DB` и накатывается `src/create_tables.sql`). Если нужны постоянные данные, стоит добавить отдельный том и бэкапы.
