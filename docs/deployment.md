...
6. В CI авторизуется в GHCR и выполняет `docker compose pull`. При локальном запуске создаёт amd64-образ командой `docker buildx build --load`, переносит его на сервер и выполняет `docker compose up -d --remove-orphans`.
