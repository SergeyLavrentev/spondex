...
6. Собирает (в локальном запуске) или подтягивает из GHCR (в CI) docker-образ и запускает `docker compose -f docker-compose.prod.yml up -d --remove-orphans` для сервиса `app`.
...
- Скопируйте шаблон `cp .env.example .env` и при необходимости измените `APP_PORT`, `EXTRA_UFW_PORTS` и другие не секретные настройки.
- Скопируйте `tokens.env.example` в `tokens.env` (или работайте сразу с `.env`) и заполните чувствительные данные в формате `KEY="value"` — сюда входят `SPOTIPY_CLIENT_ID`, `SPOTIPY_CLIENT_SECRET`, `YANDEX_TOKEN`, `TG_BOT_TOKEN` (токен телеграм-бота для мониторинга) и другие секреты. При локальном запуске плейбук читает содержимое `.env`, поэтому убедитесь, что там присутствуют все необходимые значения; в CI токен также передаётся отдельным секретом `TG_BOT_TOKEN`.
...
| `TG_BOT_TOKEN` | Токен телеграм-бота мониторинга. Передаётся в Ansible и записывается в переменную окружения сервиса. |
...
- Приложение stateless: единственное постоянное состояние — Spotify cache в `.cache`. Чтобы сбросить авторизацию, удалите этот файл и перезапустите сервис `app`.
