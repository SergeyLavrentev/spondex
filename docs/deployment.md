...
- Роль мониторинга автоматически ставит и настраивает `exim4` в режиме *internet site*, поэтому уведомления из мониторинга могут отправляться на внешние адреса. Дополнительные получатели добавляются в `/opt/spondex/monitoring/config.yaml` вручную или через переменную `monitor_mail_to`.
