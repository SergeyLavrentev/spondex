...
- Все необходимые таблицы автоматически создаются плейбуком деплоя: он проверяет том `spondex_postgres_data`, при необходимости создаёт базу и запускает `src/create_tables.sql`. Если нужно применить изменения вручную:
   ```bash
